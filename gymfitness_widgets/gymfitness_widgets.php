...
class GymFitness_Clases_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array( 'description' => esc_html__( 'Agregar las clases como Widget', 'text_domain' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
        $cantidad = !empty($instance['cantidad']) ? $instance['cantidad']:esc_html_('¿Cuántas clases deseas mostrar?', 'gymfitness');?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('cantidad')) ?>">
            <?php esc_attr_e('¿Cuántas clases deseas mostrar?','gymfitness'); ?>
            </label>
            <input 
                class="widefat" id="<?php echo esc_attr($this->get_field_id('cantidad')) ?>"
